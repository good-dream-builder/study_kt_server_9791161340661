package com.songko.simpleshopserver.domain.product.registration
...
class ProductRegistrationService @Autowired constructor(
    ...
    fun register(request: ProductRegistrationRequest) =
            ...
            userContextHolder.id?.let { userId ->
                val images by lazy {
                    findAndValidateImages(request.imageIds)
                }
                    ...
                    .also { images ->
                        images.forEach { image ->
                            if (image.productId != null) {
                                throw CustomException("이미 등록된 상품입니다.")
                            }
                        }
                    }
    ...
    private fun ProductRegistrationRequest.validateRequest() = when {
        name.length !in 1..40 ||
                ...
                price <= 0 ->
            throw CustomException("올바르지 않은 상품 정보입니다.")
        else -> {
        }
    }
